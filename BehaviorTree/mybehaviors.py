...
#Author: pmanjunath9 - Pranav Manjunath
#Collaboration: I discussed with Srajan Dube various ideas on how to structure the behavior tree, dodge the bullets, avoid getting hit by the hunter, and attacking any minions within range both while retreating and chasing.
...
def treeSpec(agent):
    ...
    spec = 	[AttackMinion,
                [(Selector, 0),
                    [(HunterVisible, 1), (Retreat, 0, 2)
                    ],
                    (Retreat, 0.5, 3),
                    [(HunterNotVisible, 4),
                        [(HitpointDaemon, 0.5, 5),
                            [(Selector, 6),
                                [(Sequence, 7),
                                    (ChaseHero, 8), (KillHero, 9)
                                ]
                            ]
                        ]
                    ]
...
def myBuildTree(agent):
    ...
    root = None
    ### YOUR CODE GOES BELOW HERE ###
    #Chose to implement treespec() instead
    ### YOUR CODE GOES ABOVE HERE ###
    return root
...
class Retreat(BTNode):
    ...
    def execute(self, delta = 0):
        ret = BTNode.execute(self, delta)
        #if self.agent.getVisibleType(Bullet) and len(self.agent.getVisibleType(Bullet)) > 0:
            #self.agent.dodge()
...
class HunterVisible(BTNode):
    ...
    def execute(self, delta = 0):
        ...
        for enemy in enemies:
            if not isinstance(enemy, Minion):
...
class AttackMinion(BTNode):
    ...
    def execute(self, delta = 0):
        ...
        enemies = self.agent.world.getEnemyNPCs(self.agent.getTeam())
        target = None
        atLeastOneMinion = False
        for enemy in enemies:
            if distance(self.agent.getLocation(), enemy.getLocation()) < AREAEFFECTRANGE * self.agent.getMaxRadius():
                atLeastOneMinion = True
            if distance(self.agent.getLocation(), enemy.getLocation()) < BIGBULLETRANGE:
                target = enemy
                break
        if self.agent is not None and target is not None:
            self.agent.turnToFace(target.getLocation())
            self.agent.shoot()
        if atLeastOneMinion and self.agent.canAreaEffect():
