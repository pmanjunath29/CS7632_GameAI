...
#Collaboration: I dicussed with Srajan Dube various ideas on how to structure the behavior tree, dodge the bullets, and avoid getting hit by the hunter.
...
class Sequence(BTNode):
	...
	def execute(self, delta = 0):
		...
		if BTNode.getNumChildren(self) > 0:
			curIndex = BTNode.getCurrentIndex(self)
			length = BTNode.getNumChildren(self)
			if curIndex >= length:
				return True
			else:
				curChild = BTNode.getChild(self, curIndex)
				succeeded = curChild.execute(delta)
				if succeeded is None:
					return None
				elif not succeeded:
					return False
				else:
					BTNode.setCurrentIndex(self, curIndex + 1)
					return None    
