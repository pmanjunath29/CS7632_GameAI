...
class Base(Mover):
	...
	def __init__(self, image, position, world, team = None, minionType = Minion, heroType = Hero, hunterType = Hero, buildrate = BUILDRATE, hitpoints = BASEHITPOINTS2, firerate = BASEFIRERATE, bulletclass = BaseBullet):
		...
		self.hunterTimer = 500
		self.hunterRate = 1000
	...
	def spawnHunter(self, angle = 0.0):
		vector = (math.cos(math.radians(angle)), -math.sin(math.radians(angle)))
		agent = self.hunterType(self.getLocation(), 0, self.world, firerate=FIRERATE*2, bulletclass=HunterBullet)
		pos = (vector[0]*(self.getRadius()+agent.getRadius())/2.0,vector[1]*(self.getRadius()+agent.getRadius())/2.0)
		agent.move(pos)
		if self.nav is not None:
			newnav = cloneAStarNavigator(self.nav)
	...
	def update(self, delta):
		...
		if self.buildTimer >= self.buildRate:
			...
			self.buildTimer = 0
		self.hunterTimer = self.hunterTimer + 1
		if self.hunterTimer >= self.hunterRate:
			self.spawnHunter()
			self.hunterTimer = 0
