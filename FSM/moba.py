...
class Base(Mover):
	...
	def __init__(self, image, position, world, team = None, minionType = Minion, hunterType = Hero, buildrate = BUILDRATE, hitpoints = BASEHITPOINTS, firerate = BASEFIRERATE, bulletclass = BaseBullet):
	...
	def spawnHunter(self, angle = 0.0):
		vector = (math.cos(math.radians(angle)), -math.sin(math.radians(angle)))
		agent = self.hunterType(self.getLocation(), 0, self.world)
		pos = (vector[0]*(self.getRadius()+agent.getRadius())/2.0,vector[1]*(self.getRadius()+agent.getRadius())/2.0)
		agent.move(pos)
		if self.nav is not None:
			newnav = cloneAStarNavigator(self.nav)
